...
public class MagnitudeEnchantment extends Enchantment implements PolymerObject {
    ...
    private static void doMagnitudeAttack(LivingEntity attacker) {
        ...
        Function<LivingEntity, DamageSource> damageSourceFunction = attacker instanceof PlayerEntity ? e -> DamageSource.player((PlayerEntity) attacker) : DamageSource::mob;
        ...
        if (attacker instanceof PlayerEntity) {
            ((DuckPlayerEntity) (PlayerEntity) attacker).setMagnitudeCooldown(COOLDOWN);
        }
        ...
        for (int i = 0; i < maxDistance * SEGMENTS_IN_BLOCK; i += 1) {
            ...
            if (doSingleAttackPortion(world, attacker, pos, i, (int) maxDistance * SEGMENTS_IN_BLOCK, lookVector, damageSource, magnitudeEffectTypes)) {
                if (attacker instanceof PlayerEntity) {
                    ((DuckPlayerEntity) (PlayerEntity) attacker).setMagnitudeCooldown(COOLDOWN * 2);
                }
